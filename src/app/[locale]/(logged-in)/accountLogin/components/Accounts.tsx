...
import { InfoAccounts } from "./moskData";
...
const Accounts = ({ dataAccounts }: Props) => {
  ...
  const handleSelectPlan = (planAccount, id: any) => {
    setAccountSelected(planAccount);
  ...
  const searchUsername = async (idSponsor) => {
    ...
    const ownerOf = await contract.call("ownerOf", [idSponsor]);
    console.log(ownerOf);
    ...
    const buscarNfts = async () => {
      ...
      try {
        ...
        while (true) {
          ...
          const nftImage = await contract.call("nftImage", [parseInt(arrayInfo._hex, 16)]);
          ...
          for (let index = 0; index < parseInt(getDirectVolCount._hex, 16); index++) {
            ...
            directVol = directVol + parseFloat(ethers.utils.formatEther(getDirectVolInfo[1]));
          }
          ...
          for (let index = 0; index < parseInt(getCantLevles._hex, 16); index++) {
            ...
            for (let x = 0; x < parseInt(getDirectVolInfo._hex, 16); x++) {
              ...
              generalVol = generalVol + parseFloat(ethers.utils.formatEther(getGlobalVolInfo[1]));
            }
          }
          ...
          if (parseInt(accountInfo.idAccountRefferal._hex, 16) != 0) {
            ...
            arrayNombreRefferal.push("Master Account");
          }
          ...
          console.log(parseInt(nftImage._hex,16))
          arrayFotos.push(parseInt(nftImage._hex,16));
          i++;
  ...
  return (
    <div className="h-screen bg-gradient-to-t from-[#0E0E33] to-[#39307B] pb-24 ">
          ...
          {nfts.status ? (
            <>
              {nfts.data.map((account, index) => (
                <div className="rounded-[10px]  h-[145px]" key={index} onClick={() => handleSelectPlan(account, account.idAccount)}>
                  <div className={` ${index === 0 ? "bg-[#7573A6]" : "bg-[#70D1F4]"}  rounded-t-[10px] h-[10px]`}></div>
                  <div className="py-1 text-center bg-[#ffffff1a] w-full h-full px-3 flex flex-col items-center justify-center">
                    <div className="rounded-full border-[4px] border-solid border-[#ffffff1a] w-[60px] h-[60px]">
                    <Image src={listNftBuy[nftImg[index] - 1].img} alt="logo" width={60} height={60} className="rounded-full" />
                    </div>
                    <p className="text-white text-[10px] font-bold rounded-[20px] bg-[#ffffff1a] py-1 px-[10px] mt-4 w-[120px]">
                   {parseInt(account.idAccount._hex,16)} | {account.accountName}
                    <p className="text-[#A9AEB4] text-[8px]">{membresiaAlta[index].replace(/['`]/g, "")}</p>
                   {/* <p className="text-[#A9AEB4] text-[8px]">{nftImg[index]}</p>*/}
                    </p>
                  </div>
                </div>
                //    <div className="rounded-[10px]" key={index} onClick={() => handleSelectPlan(account, account.idAccount)}>
                //    <div className="bg-[#9B111E] rounded-t-[10px] h-[10px]"></div>
                //    <div className="py-1 text-center bg-[#ffffff1a]">
                //      <h1 className="text-white text-[12px] font-bold mb-1">{account.accountName}</h1>
                //      <p className="text-[#A9AEB4] text-[8px]">{membresiaAlta[index].replace(/['`]/g, "")}</p>
                //    </div>
                //    <div className="py-2 text-center bg-[#554D77]">
                //      <div>
                //        <p className="text-white text-[8px] mb-1">{t("DIRECTS VOL")}</p>
                ...
                //          </p>
                //          <p className="text-[9px] font-bold text-white ml-4">${volDirect[index]}</p>
                //        </div>
                //      </div>
                //      <div className="mt-2">
                //        <p className="text-white text-[8px] mb-1">{t("GLOBAL VOL")}</p>
                //        <div className="flex items-center justify-center">
                //          <p className="text-[#7A2FF4] text-[8px] font-bold flex items-center justify-center w-[16px] h-[16px] rounded-full bg-white">
                //            {account.dineroGlobal}
                //          </p>
                //          <p className="text-[9px] font-bold text-white ml-4">${volGlobal[index]}</p>
                //        </div>
                //      </div>
                //    </div>
                //    <div className="bg-[#ffffff1a] rounded-b-[10px] py-2">
                //      <div className="text-white text-[8px] font-bold text-center">
                //        <span>{t("Sponsor")}: </span>
                //        <span>{nombreRefferal[index]}</span>
                //      </div>
                //      <div className="text-[#A9AEB4] text-[8px] text-center mt-1">
                //        <span>{t("SINCE")} </span>
            ...
            <>
              <center className="text-white text-[14px] mt-4 col-span-2">{t("Loading")}...</center>
            </>
          )}
        </div>
      </div>
